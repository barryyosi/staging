...
const cache = new Map();
// Lookups already running, so two callers asking at once share one CLI run
...
const PROVIDERS = {
  ...
  azure: {
    ...
    // On Windows `az` is a .cmd shim, which Node refuses to spawn without a
    // shell (CVE-2024-27980), and a shell is unsafe with arbitrary branch
    // names. Azure detection is therefore macOS/Linux only for now.
    cli: 'az',
    args: (branch) => [
      'repos',
      'pr',
...
async function findOpenPullRequest(
  ...
  const lookup = lookupPullRequest(gitRoot, branch, forcedProvider).finally(
    () => inFlight.delete(key),
  );
...
async function lookupPullRequest(gitRoot, branch, forcedProvider) {
