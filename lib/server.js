...
export function startServer({
  ...
  // Git-backed routes are meaningless in standalone preview mode — reject
  // them with a clear error instead of surfacing git failures.
  const GIT_ONLY_ROUTES = new Set([
    '/api/diff',
    '/api/commit',
    '/api/push',
    '/api/project-info',
  ...
  app.use('/api/*', async (c, next) => {
    if (previewMode && GIT_ONLY_ROUTES.has(c.req.path)) {
  ...
  const validatePath = (p) => {
    ...
    try {
      ...
      return (
        absolutePath === absoluteGitRoot ||
        absolutePath.startsWith(absoluteGitRoot + path.sep)
      );
    } catch {
      return false;
    }
  ...
  app.get('/api/file-content', (c) => {
    try {
      ...
      const content = previewMode
        ? fs.readFileSync(path.resolve(gitRoot, filePath), 'utf-8')
        : getStagedFileContent(gitRoot, filePath);
  ...
  app.get('/api/raw-file', (c) => {
    try {
      ...
      const buffer = previewMode
        ? fs.readFileSync(path.resolve(gitRoot, filePath))
        : getStagedFileBuffer(gitRoot, filePath);
